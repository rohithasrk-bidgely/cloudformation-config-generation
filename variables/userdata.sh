...
echo
echo "Configuring repo2.bidgely.com/prodna"
#deb http://repo.bidgely.com prodna/
# Create Source list for packages to install from our local repo
echo "deb http://repo2.bidgely.com prodna/" >> /etc/apt/sources.list
echo "Package: *" >> /etc/apt/preferences
echo 'Pin: origin "repo2.bidgely.com"' >> /etc/apt/preferences
...
aws s3 cp s3://bidgely-devops/spot/s3mover /opt/bidgely/s3mover
aws s3 cp s3://bidgely-devops/spot/s3move_logs /etc/cron.d/
...
echo BIDGELY_ENV=prod-na >> /etc/environment;
...
aws s3 cp s3://$S3ARTIFACTSBUCKET/prodna-rpmlist .
PACKAGELIST=`cat prodna-rpmlist | grep -w aggregations-spot | awk -F \| '{print $2}' |tr "," " "`

export DEBIAN_FRONTEND=noninteractive
echo "Unstalling the following packages "
...
if [ "NO" == "YES" ]; then
    ...
    aws s3 cp s3://bidgely-devops/spot/cloudwatch-logpush.sh /opt/bidgely/cloudwatch-logpush.sh
